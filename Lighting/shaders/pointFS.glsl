...
vec4 phong(Light light, vec3 surface_normal, vec3 surface_color)
{
	...
	outputColor = vec4((surface_color*ambient*diffuse)+specular, 1.f);
...
{		
	...
	{
		result += phong(g_light[i], tnorm, fVertexColor);
	}
